...
void enclaveChangeBuffer(unsigned long long start_addr, unsigned long long end_addr) {

	unsigned char symbol = 0;
	unsigned long long checksum = 0;
	for (unsigned long long i = start_addr; i < end_addr; i++)
	{
		memcpy(&symbol, (void*)i, 1);
		checksum += symbol;
	}
	printf("checksum: %llu", checksum);
}
